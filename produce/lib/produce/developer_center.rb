      ...
      if app_exists?
        ...
        Produce.config[:enabled_features].each do |k, v|
          if k == :data_protection
            ...
            when "unlessopen"
              enabled_clean_options[app_service.data_protection.unless_open.service_id] = app_service.data_protection.unless_open
            when "untilfirstauth"
              enabled_clean_options[app_service.data_protection.until_first_auth.service_id] = app_service.data_protection.until_first_auth
            end
          elsif k == :icloud
            case v
            when "legacy"
